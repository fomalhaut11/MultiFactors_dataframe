...
class GTJA_191:
 ...
 ##################################################################
    ...
    #     condition2=(self.close.rolling(window=2,min_periods=1).sum()/2<(self.close.rolling(window=8,min_periods=1).sum()/8-self.close.rolling(window=8,min_periods=1).std()))
    ...
    def alpha_004(self, window1=8, window2=2, window3=20):
        ...
        condition1=(self.adj_close.rolling(window1,min_periods=1).std()<self.adj_close.rolling(window2,min_periods=1).sum()/2)
        condition2=(self.adj_close.rolling(window2,min_periods=1).sum()/2<(self.adj_close.rolling(window1,min_periods=1).sum()/8-self.adj_close.rolling(window1,min_periods=1).std()))
        condition3=(1<=self.volume/self.volume.rolling(window3,min_periods=1).mean())
        indicator1 = pd.DataFrame(np.ones(self.close.shape), index=self.close.index,
                                  columns=self.close.columns)  # [condition2]
        indicator2 = -pd.DataFrame(np.ones(self.close.shape), index=self.close.index,
                                   columns=self.close.columns)  # [condition3]
 ...
 ##################################################################
    ...
    def alpha_005(self):
        ...
        ts_volume = self.volume.rolling(window=7).apply(rank_pct)
        ts_high=self.high.rolling(window=7).apply(rank_pct)
        corr_ts = ts_high.rolling(window=5).corr(ts_volume).shift()
    ...
    #     alpha = result.iloc[-1, :]
    #     return alpha.dropna()
    def alpha_007(self, window=3):
        part1 = (np.maximum(self.avg_price - self.close, window)).rank(axis=1, pct=True)
    ...
    #     vwap10=self.avg_price.rolling(window=10,min_periods=1).mean()
    ...
    #     part=temp1.rolling(window=5,min_periods=1).corr(temp2)
    ...
    #     A=self.close.rolling(window=6,min_periods=1).mean().iloc[-6:, :]
    #     B = np.arange(1, 7)
    ...
    #     alpha = result.iloc[-1, :]
    #     return alpha.dropna()
    def alpha_022(self, window=6):
        part1 = (self.close- self.close.rolling(window, min_periods=1).mean()) / self.close.rolling(window, min_periods=1).mean()
        temp = (self.close - self.close.rolling(window, min_periods=1).mean()) / self.close.rolling(window, min_periods=1).mean()
        part2 = temp.shift(3)
        result = part1 - part2
    ...
    #     alpha = part1 + part2
    #     return alpha.dropna()
    def alpha_026(self, window=7, window2=230):
    ...
    def alpha_033(self, window=5, window2=240, window3=20):
        ...
        part1 = temp1.shift(5) - temp1
        temp2 = (ret.rolling(window2,min_periods=1).sum() - ret.rolling(window3,min_periods=1).sum()) / 220
        part2 = temp2.rank(axis=1, pct=True)
    ...
    #     result = self.close.rolling(window=12).mean() / self.close
    #     alpha = result.iloc[-1, :]
    #     return alpha.dropna()
    def alpha_034(self, window=12):
    ...
    def alpha_035(self):
        ...
        part1 =temp1.rolling(window=n).apply(lambda x:weighted_sum(x,weight1),raw=False)
        part1 = part1.rank(axis=1, pct=True)
    ...
    #     temp = self.open_price.rolling(window=5).sum() * ret.rolling(window=5).sum()
    ...
    #     sum_20 = self.high.rolling(window=20).mean()
    ...
    #     volume_180 = self.volume.rolling(window=180).mean()
    #     #sum_vol = pd.rolling_sum(volume_180, 37)
    #     sum_vol=volume_180.rolling(window=37).sum()
    ...
    #     vol_sum = vol.rolling(window=26).sum()
    #     vol1 = self.volume[~condition].fillna(0)
    #   #  vol1_sum = pd.rolling_sum(vol1, 26)
    #     vol1_sum = vol1.rolling(window=26).sum()
    ...
    #     part2 = self.high.rolling(window=10).std()
    ...
    #     temp2 = self.volume.rolling(window=150).mean()
    #   #  part2 = pd.rolling_corr(self.avg_price, temp2, 15)
    #     part2 = self.avg_price.rolling(window=15).corr(temp2)
    #     part2 = part2.rank(axis=1, pct=True)
    #     result = part1 * part2
    #     alpha = result.iloc[-1, :].dropna()
    #     return alpha
    ...
    #     part1 = self.close.rolling(window=3,min_periods=1).mean()
    #     part2 = self.close.rolling(window=6,min_periods=1).mean()
    #     part3 = self.close.rolling(window=12,min_periods=1).mean()
    #     part4 = self.close.rolling(window=24,min_periods=1).mean()
    ...
    #     result = -summ *self.volume.rolling(window=5,min_periods=1).sum() / self.volume.rolling(window=20,min_periods=1).sum()
    #     alpha = result.iloc[-1, :].dropna()
    #     return alpha
    def alpha_048(self):
        condition1 = (self.close > self.close.shift())
    ...
    def alpha_049(self):
        ...
        part1 = np.maximum(np.abs(self.high - delay_high), np.abs(self.low - delay_low))
        part1 = part1[~condition1]
        part1 = part1.rolling(window=12).sum()
        part2 = np.maximum(np.abs(self.high - delay_high), np.abs(self.low - delay_low))
        part2 = part2[~condition2]
        part2 = part2.rolling(window=12).sum()
        result = part1 / (part1 + part2)
        alpha = result.shift() 
        return alpha.stack()
    ##################################################################
    ...
    def alpha_052(self):
        delay = ((self.high + self.low + self.close) / 3).shift()
        part1 = (np.maximum(self.high - delay, 0)).rolling(window=26).sum()
        part2 = (np.maximum(delay - self.low, 0)).rolling(window=26).sum()
        alpha = part1 + part2 
        return alpha.shift().stack()
    ##################################################################
    # def alpha_053(self):
    ...
    def alpha_053(self):
        ...
        alpha = result.rolling(window=12).count() * 100 / 12
    ...
    def alpha_054(self):
        ...
        part1 =part2.abs().rolling(window=10).std()
        part3 = self.close.rolling(window=10).corr(self.open_price)
    ...
    #     part1 = self.close - self.low.rolling(window=9).min()
    #     #part2 = pd.rolling_max(self.high, 9) - pd.rolling_min(self.low, 9)
    #     part2 = self.high.rolling(window=9).max() - self.low.rolling(window=9).min()
    ...
    def alpha_058(self):
        delay = self.close.shift()
        condition = self.close > delay
        result = self.close[condition].rolling(window=20).count()*100/20
        alpha = result.shift() 
        return alpha.stack()
        ##################################################################
    ...
    def alpha_059(self):
        ...
        result = self.close - part1 - part2
        alpha = result.rolling(window=20).sum().shift()
        return alpha.stack()
    ##################################################################
    # def alpha_060(self):
    ...
    #     temp2 = temp2.rolling(window=8,min_periods=1).corr(self.volume.rolling(window=80,min_periods=1).mean())
    ...
    def alpha_061(self):
        ...
        temp2=self.low
        temp2 = temp2.rolling(window=8).corr(self.volume.rolling(window=80).mean())
        temp2 = temp2.rank(axis=1, pct=1)
        part2 = temp2.rolling(window=m).apply(lambda x:np.sum(x * weight2))
        part2 = -part2.rank(axis=1, pct=1)
        merged=part1.combine_first(part2)
    ...
    def alpha_062(self):
        volume_rank = self.volume.rank(axis=1, pct=1)
        result = self.high.rolling(window=5).corr(volume_rank)
        alpha = -result.shift()
        return alpha.stack()
    ...
    #     part2 = par2.ewma(alpha=1.0 / 6).mean()
    ...
    #     temp1 = self.avg_price.rank(axis=1,pct=1).rolling(window=4,min_periods=1).corr(self.volume.rank(axis=1,pct=1))
    #     temp1 = temp1.iloc[-n:, :]
    #     seq1 = [2 * i / (n * (n + 1)) for i in range(1, n + 1)]
    #     seq2 = [2 * i / (m * (m + 1)) for i in range(1, m + 1)]
    #     weight1 = np.array(seq1)
    ...
    #     temp2 = temp2.rolling(window=4).corr(self.volume.rolling(window=60).mean())
    ...
    def alpha_064(self):
        ...
        temp2=self.close.rank(axis=1,pct=1).rolling(window=4).corr(self.volume.rolling(window=60).mean())
        temp2 = np.maximum(temp2, 13)
        part2 = temp2.rolling(window=m).apply(lambda x:np.sum(x * weight2))
        part2 = -part2.rank(axis=1, pct=1)
    ...
    def alpha_065(self):
        ...
        alpha = self.close.rolling(window=6).mean() / self.close
        alpha = alpha.shift()
        return alpha.stack()
    ##################################################################
    # def alpha_066(self):
    ...
    #     return alpha
    def alpha_066(self):
        part1 = self.close.rolling(window=6).mean()
    ...
    def alpha_070(self):
        #### STD(AMOUNT, 6)
        ##
        alpha = self.amount.rolling(window=6).std().shift()
        return alpha.stack()
    #############################################################################
    # def alpha_071(self):
    ...
    #     data = self.close - self.close.rolling(window=24).mean() / self.close.rolling(window=24).mean()
    ...
    #     data1 = self.high.rolling(window=6).max() - self.close
    #    # data2 = pd.rolling_max(self.high, 6) - pd.rolling_min(self.low, 6)
    #     data2 = self.high.rolling(window=6).max() - self.low.rolling(window=6).min()
    ...
    def alpha_072(self):
       ...
       # data2 = pd.rolling_max(self.high, 6) - pd.rolling_min(self.low, 6)
        data2 = self.high.rolling(window=6).max() - self.low.rolling(window=6).min()
       # alpha = pd.ewma(data1 / data2 * 100, alpha=1 / 15).iloc[-1].dropna()
        alpha = (data1 /data2 *100).ewm(alpha=1/15).mean().shift()
        return alpha.stack()
    #############################################################################
    def alpha_073(self):
        raise NotImplementedError
    ...
    def alpha_076(self):
        ...
        data1 = data0.rolling(window=20).std()
        data2 = data0.rolling(window=20).mean()
    ...
    #     data2 = ((self.high + self.low) / 2).rolling(window=3).corr(self.volume.rolling(window=40)).iloc[-6:, :]
    ...
    def alpha_077(self):
        ...
        decay_weights = np.arange(1, 20 + 1, 1)[::-1]
        decay_weights = decay_weights / decay_weights.sum()
        rank1=data1.rolling(window=20).apply(lambda x:np.sum(x * decay_weights)).rank(axis=1, pct=True)
 
        data2_0=(self.high + self.low) / 2
        data2_1=self.volume.rolling(window=40).mean()
        data2 =data2_0.rolling(window=3).corr(data2_1)
 
        decay_weights2 = np.arange(1, 6 + 1, 1)[::-1]
        decay_weights2 = decay_weights2 / decay_weights2.sum()
        rank2=data2.rolling(window=6).apply(lambda x:np.sum(x * decay_weights2)).rank(axis=1, pct=True)
    ...
    def alpha_078(self):
        ...
        data2 = abs(self.close - data20.rolling(window=12).mean())
        data3 = data2.rolling(window=12).mean() * 0.015
    ...
    def alpha_080(self):
        # (VOLUME-DELAY(VOLUME,5))/DELAY(VOLUME,5)*100
        #
        alpha = ((self.volume - self.volume.shift(5)) / self.volume.shift(5) * 100).shift()
        return alpha.stack()
    #############################################################################
    # def alpha_081(self):
    ...
    # def alpha_082(self):
    #  #   part1 = pd.rolling_max(self.high, 6) - self.close
    #     part1 = self.high.rolling(window=6).max() - self.close
    #   #  part2 = pd.rolling_max(self.high, 6) - pd.rolling_min(self.low, 6)
    #     part2 = self.high.rolling(window=6).max() - self.low.rolling(window=6).min()
    ...
    #     return alpha.dropna()
    def alpha_083(self):
        part1 = self.high.rank(axis=1, pct=True)
        part2 = self.volume.rank(axis=1, pct=True)
       
        result = part1.rolling(window=5).corr(part2)
        alpha = -result.shift()
        return alpha.stack()
    #############################################################################
    # def alpha_084(self):
    ...
    #     return alpha.dropna()
    def alpha_085(self):
        temp1mean=self.volume.rolling(window=20).mean()
        temp1 = self.volume/temp1mean
        part1 = temp1.rank(axis=1, pct=True)
        
        delta = self.close.diff(7)
        temp2=delta.rolling(window=8).apply(lambda x:x.rank(pct=True).iloc[-1])
        part2 = temp2 
        part2 = part2
        alpha = part1 * part2
        alpha = alpha.shift()
    ...
    def alpha_088(self):
        ...
        alpha=(self.close-self.close.shift(21))/self.close.shift(21)*100
        alpha = alpha.shift()
        return alpha.stack()
    # def alpha_89(self):
    ...
    def alpha_090(self):
        ...
        data1 = self.avg_price.rank(axis=1, pct=True)
        data2 = self.volume.rank(axis=1, pct=True)
        corr = data1.rolling(window=5).corr(data2)
        rank1 = corr.rank(pct=True)
        alpha = rank1 * -1
        alpha = alpha.shift()
        return alpha.stack()
    ...
    def alpha_091(self):
        # ((rank((close-max(close,5)))*rank(corr((mean(volume,40)),low,5)))*-1)
        #################
        data1 =self.close- self.close.rolling(window=5).max()
        rank1=data1.rank(axis=1,pct=True)
 
       # mean = pd.rolling_mean(self.volume, window=40)
        mean = self.volume.rolling(window=40).mean()
        corr = mean.rolling(window=5).corr(self.low)
        rank2 = corr.rank(axis=1, pct=True)
        alpha = rank1 * rank2 * (-1)
        alpha = alpha.shift()
        return alpha.stack()
    # def alpha_92(self):
    ...
    def alpha_092(self):
        ...
        rank1=delta.rolling(window=3).apply(self.func_decaylinear).rank(axis=1,pct=True)
        
        vm1=self.volume.rolling(window=180).mean()
        cor1=vm1.rolling(13).corr(self.close).abs()
        cor1=cor1.replace([np.inf,-np.inf],1)
        cor1=cor1.fillna(0)
        data1=cor1.rolling(window=5).apply(self.func_decaylinear)
        rank2=data1.rolling(window=15).apply(lambda x:x.rank(pct=True)[-1])
    ...
    def alpha_093(self):
        ...
        data2[cond_max] = data1[cond_max]
        data2[cond] = 0
        alpha = data2.rolling(window=20).sum()
        alpha = alpha.shift()
        return alpha.stack()
    # def alpha_94(self):
    ...
    def alpha_094(self):
        ...
        value[~cond2] = 0
        value[cond1] = self.volume[cond1]
        alpha = value.rolling(window=30).sum()
        alpha = alpha.shift()
        return alpha.stack()
    # def alpha_95(self):
    #     # STD(AMOUNT,20) #
    ...
    def alpha_095(self):
        # STD(AMOUNT,20) #
        alpha = self.amount.rolling(window=20).std()
        alpha = alpha.shift()
        return alpha.stack()
    # def alpha_96(self):
    ...
    def alpha_096(self):
        ...
        alpha = sma1.ewm(span=5,adjust=False).mean() 
        alpha = alpha.shift()
        return alpha.stack()
    # def alpha_97(self):
    #     # STD(VOLUME,10) #
    ...
    def alpha_097(self):
        # STD(VOLUME,10) #
        alpha = self.volume.rolling(window=10).std()
        alpha = alpha.shift()
        return alpha.stack()
    # def alpha_98(self):
    ...
    #     alpha = (-self.close.rank(axis=1, pct=True).rolling(window=5).cov(self.volume.rank(axis=1, pct=True).rolling(window=5))
    #       .rank(axis=1, pct=True).iloc[-1, :])
    ...
    def alpha_099(self):
        ...
        alpha = (-rank1.rolling(window=5).cov(rank2)).rank(axis=1, pct=True)
    ...
    def alpha_100(self):
        # STD(VOLUME,20) #
        alpha = self.volume.rolling(window=20).std()
        alpha = alpha.shift()
        return alpha.stack()
    #
    # def alpha_101(self):
    ...
    #     return alpha
    def alpha_103(self):
        data1=self.low.rolling(window=20).apply(self.func_lowday)
        data1=(20-data1)/20*100
        alpha=data1.shift()
        return alpha.stack()
    ...
    def alpha_104(self):
        # (-1*(DELTA(CORR(HIGH,VOLUME,5),5)*RANK(STD(CLOSE,20)))) #
        corr = self.high.rolling(window=5).corr(self.volume)
        alpha = (-(corr - corr.shift(5)) * ((self.close.rolling(window=20).std()).rank(axis=1, pct=True))) 
        alpha = alpha.shift()
        return alpha.stack()
    #
    # def alpha_105(self):
    ...
    def alpha_105(self):
        ...
        temp2=self.volume.rank(axis=1, pct=True)
        alpha =  temp1.rolling(window=10).corr(temp2)
        alpha = alpha.shift()
        return alpha.stack()
    #
    # def alpha_106(self):
    ...
    #     rank1 = data1.rolling(window=20).mean().rank(axis=1, pct=True)
    ...
    def alpha_113(self):
        ...
        rank1 = data1.rolling(window=20).mean().rank(axis=1, pct=True)
        corr1 = self.close.rolling(window=2).corr(self.volume)
   
        data2 = self.close.rolling(window=5).sum()
        data3 = self.close.rolling(window=20).sum()
        corr2 = data2.rolling(window=2).corr(data3)
    ...
    def alpha_115(self):
        ...
        data1 = (self.high * 0.9 + self.close * 0.1)
        data2 = self.volume.rolling(window=30).mean()
        rank1=data1.rolling(window=10).corr(data2).rank(axis=1,pct=True)
        data1=((self.high+self.low)/2)
        tsrank1=data1.rolling(4).apply(lambda x:x.rank(axis=0,pct=True)[-1])
         
        tsrank2=self.volume.rolling(window=10).apply(self.func_rank)
        rank2=tsrank1.rolling(window=7).corr(tsrank2).rank(axis=1,pct=True)
    ...
    def alpha_116(self):
        # REGBETA(CLOSE,SEQUENCE,20) #
        sequence = pd.Series(range(1, 21), index=self.close.iloc[-20:, ].index)  # 1~20
        corr=self.close.rolling(window=20).corr(sequence)
    ...
    def alpha_117(self):
        ...
        data2=1 -data1.rolling(window=16).rank(pct=True)
        data3 =self.volume.rolling(window=32).rank(pct=True)
        ret = (self.close / self.close.shift() - 1) 
        data4 = 1 - ret.rolling(window=32).rank(pct=True)
    ...
    def alpha_122(self):
        ...
        ##
        log_close = np.log(self.close)
        data1=log_close.ewm(span=12,adjust=False).mean()
        data2=data1.ewm(span=12,adjust=False).mean()
        data=data2.ewm(span=12,adjust=False).mean()
    ...
    def alpha_123(self):
        ...
        data1 = ((self.high + self.low) / 2).rolling(20).sum()
        data2 = self.volume.rolling(60).mean().rolling(20).sum()
        corr1=data1.rolling(window=9).corr(data2)
        rank1=corr1.rolling(window=9).apply(lambda x : x.rank(axis=0,pct=True)[-1])
        
        corr2=self.low.rolling(window=6).corr(self.volume)
        rank2=corr2.rolling(window=9).apply(lambda x : x.rank(axis=0,pct=True)[-1])
        rank1 = rank1.reindex(rank2.index).reindex(columns=rank2.columns)
        cond_max=rank1>rank2
        rank2[cond_max]=rank1[cond_max]
    ...
    def alpha_125(self):
        ##### (RANK(DECAYLINEAR(CORR((VWAP), MEAN(VOLUME, 80), 17), 20)) / RANK(DECAYLINEAR(DELTA((CLOSE * 0.5 + VWAP * 0.5), 3), 16)))
        ##
        vmean=self.volume.rolling(80).mean()
        data1=self.avg_price.rolling(window=17).corr(vmean)
        rank1=data1.rolling(window=20).apply(self.func_decaylinear)
        rank1=rank1.rolling(window=20).apply(self.func_rank)

        data2 = (self.close * 0.5 + self.avg_price * 0.5).diff(3)
 
        rank2=data2.rolling(window=16).apply(self.func_decaylinear)
        rank2=rank2.rolling(window=16).apply(self.func_rank)
        alpha = rank1 / rank2
        alpha = alpha.shift()
        return alpha.stack()
    # def alpha_126(self):
    ...
    def alpha_129(self):
        ...
        data[data >= 0] = 0
        data = abs(data)
        alpha = data.rolling(window=12).apply(self.func_decaylinear)
        alpha = alpha.shift()
        return alpha.stack()
    # def alpha_130(self):
    #     #### alpha_130
    ...
    def alpha_130(self):
        ...
        data2 = self.volume.rolling(40).mean()
        data3 = data1.rolling(window=9).corr(data2)
        data3 = data3.rolling(window=9).apply(self.func_decaylinear)
        rank1=data3.rolling(window=9).apply(self.func_rank)
        ...
        data3 = data1.rolling(window=7).corr(data2)
        data3 = data3.rolling(window=9).apply(self.func_decaylinear)
        rank2 =data3.rolling(window=9).apply(self.func_rank)
    ...
    def alpha_132(self):
        #### MEAN(AMOUNT, 20)
        ##
        alpha = self.amount.rolling(window=20).mean()
        alpha = alpha.shift()
        return alpha.stack()
    # def alpha_133(self):
    #     #### alpha_133
    ...
    def alpha_134(self):
        #### (CLOSE - DELAY(CLOSE, 12)) / DELAY(CLOSE, 12) * VOLUME
        ##
        alpha = ((self.close  / self.close.shift(13) - 1) * self.volume)
        alpha = alpha.shift()
        return alpha.stack()
    # def alpha_135(self):
    ...
    def alpha_135(self):
        #### SMA(DELAY(CLOSE / DELAY(CLOSE, 20), 1), 20, 1)
        ##
        c20=self.close/self.close.shift(20)
        alpha=c20.ewm(com=19,adjust=False).mean()
        alpha=alpha.shift()
        return alpha.stack()
    ...
    def alpha_136(self):
        ...
        data2=self.open_price.rolling(10).corr(self.volume)
        alpha =data1*data2
        alpha=alpha.shift()
    ...
    def alpha_138(self):
        ...
        ##
        data1 = (self.low * 0.7 + self.avg_price * 0.3).diff(3)
        rank1= data1.rolling(20).apply(self.func_decaylinear)
        rank1=rank1.rolling(20).apply(self.func_rank)
    ...
    def alpha_139(self):
        #### (-1 * CORR(OPEN, VOLUME, 10))
        ##
        alpha = - self.open_price.rolling(10).corr(self.volume)
        alpha=alpha.shift()
        return alpha.stack()
    # def alpha_140(self):
    ...
    def alpha_140(self):
        ...
        data1 = self.close.rolling(8).apply(self.func_rank)
        data2 = self.volume.rolling(60).mean().rolling(20).apply(self.func_rank)
        data3 = data1.rolling(8).corr(data2)
        data3 = data3.rolling(7).apply(self.func_decaylinear)
    ...
    def alpha_141(self):
        ...
        df1 = self.high.rank(axis=1, pct=True)
        df2 = self.volume.rolling(15).mean().rank(axis=1, pct=True)
        alpha = df1.rolling(9).corr(df2).rank(pct=True)*-1
        alpha = alpha.shift()
        return alpha.stack()
    # def alpha_142(self):
    ...
    def alpha_144(self):
        ...
        ##
        df1 = self.close < self.prev_close
        sumif = ((abs(self.close / self.prev_close - 1) / self.amount) * df1).rolling(20).sum()
        count = df1.rolling(20).sum()
    ...
    #     data1 = (self.close / self.close.shift(9)).shift().ewm(span=17, adjust=False).mean().shift(12)
    #     data2 = (self.close / self.close.shift(9)).shift().ewm(span=17, adjust=False).mean().shift(26)

    #    # alpha = (pd.ewma(data1 - data2, span=17, adjust=False)).iloc[-1, :]
    #     alpha = (data1-data2).ewm(span=17,adjust=False).iloc[-1,:]
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_152(self):
        # SMA(MEAN(DELAY(SMA(DELAY(CLOSE/DELAY(CLOSE,9),1),9,1),1),12)-MEAN(DELAY(SMA(DELAY(CLOSE/DELAY(CLOSE,9),1),9,1),1),26),9,1) #
       ...
       # alpha = (pd.ewma(data1 - data2, span=17, adjust=False)).iloc[-1, :]
        alpha = (data1-data2).ewm(span=17,adjust=False).mean()
        alpha = alpha.shift()
        return alpha.stack() 
    ######################## alpha_153 #######################
    #
    ...
    #       self.close.rolling(window=12).mean() + self.close.rolling(window=24).mean()) / 4).tail(1)
    ...
    #     alpha =(self.avg_price-self.avg_price.rolling(window=16).min()).iloc[-1,:]<self.avg_price.rolling(window=180).mean().iloc[-18:,:].corrwith(self.avg_price.iloc[-18:,:])
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_154(self):
        raise NotImplementedError
    ...
    def alpha_155(self):
        ...
        sma1 = self.volume.ewm(span=13, adjust=False).mean()
        sma2 = self.volume.ewm(span=27, adjust=False).mean()
        sma1 = (sma1-sma2).ewm(span=10, adjust=False).mean()
    ...
    def alpha_156(self):
        # (MAX(RANK(DECAYLINEAR(DELTA(VWAP,5),3)),RANK(DECAYLINEAR(((DELTA(((OPEN*0.15)+(LOW*0.85)),2)/((OPEN*0.15)+(LOW*0.85)))*-1),3)))*-1 #
        delta1=self.avg_price.diff(5)
        rank1=delta1.rolling(3).apply(self.func_decaylinear).rank(axis=1,pct=True)
        delta2= (self.open_price*0.15+self.low*0.85).diff(2)/(self.open_price*0.15+self.low*0.85)
        rank2=delta2.rolling(3).apply(self.func_decaylinear).rank(axis=1,pct=True)
    ...
    #     x = ((self.close - data1.rolling(window=6).sum())/((data2-data1).rolling(window=6).sum()))*12*24
    #     # 计算出公式核心部分y
    #     #y = ((self.close - pd.rolling_sum(data1, 12)) / pd.rolling_sum((data2 - data1), 12)) * 6 * 24
    #     y=((self.close-data1.rolling(window=12).sum())/(data2-data1).rolling(window=12).sum())*6*24
    #     # 计算出公式核心部分z
    #     #z = ((self.close - pd.rolling_sum(data1, 24)) / pd.rolling_sum((data2 - data1), 24)) * 6 * 24
    #     z=((self.close-data1.rolling(window=24))/((data2-data1).rolling(window=24).sum()))*6*24
    ...
    #     data1 = self.close.rolling(window=20).std()
    #     cond = self.close <= self.close.shift(0)
    #     data1[~cond] = 0
    #  #   data2 = pd.ewma(data1, span=39)
    #     data2= data1.ewm(span=39,adjust=False).mean()
    #     alpha = data2.iloc[-1, :]
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_160(self):
    ...
    #     alpha = data1.rolling(window=12).mean().iloc[-1,:]
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_161(self):
        ###########mean((max(max(high-low),abs(delay(close,1)-high)),abs(delay(close,1)-low)),12)
    ...
    #     x=data1.ewm(span=23,adjust=False).mean()
    #     data2 = pd.Series.abs(self.close - self.close.shift())
    #    # y = pd.ewma(data2, span=23)
    #     y= data2.ewm(span=23,adjust=False).mean()
    #     z = (x / y) * 100
    #     cond = z > 12
    #     z[cond] = 12
    #     c = (x / y) * 100
    ...
    #     alpha = data3.ewm(span=12,adjust=False).mean().iloc[-1,:]
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_164(self):
        ################
    ...
    #     data2 = data1.rolling(window=12).sum().iloc[-1,:]
    #     alpha = data2
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_167(self):
        ...
        data1[cond] = 0
        #data2 = (pd.rolling_sum(data1, 12)).iloc[-1, :]
        data2 = data1.rolling(window=12).sum()
        alpha = data2
        alpha = alpha.shift()
        return alpha.stack()
    # def alpha_168(self):
    ...
    #     data1 = (-1 * self.volume) / self.volume.rolling(window=20).mean()
    #     alpha = data1.iloc[-1, :]
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_168(self):
    ...
    #     data2 = data1.ewm(span=17,adjust=False).mean().shift()
    #    # data3 = pd.rolling_mean(data2, 12) - pd.rolling_mean(data2, 26)
    #     data3 = data2.rolling(window=12).mean()-data2.rolling(window=26).mean()
    #     #alpha = (pd.ewma(data3, span=19)).iloc[-1, :]
    #     alpha = data3.ewm(span=19,adjust=False).mean().iloc[-1,:]
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_169(self):
        ##
       ...
       # data3 = pd.rolling_mean(data2, 12) - pd.rolling_mean(data2, 26)
        data3 = data2.rolling(window=12).mean()-data2.rolling(window=26).mean()
        #alpha = (pd.ewma(data3, span=19)).iloc[-1, :]
        alpha = data3.ewm(span=19,adjust=False).mean() 
        alpha = alpha.shift()
        return alpha.stack()
    # def alpha_170(self):
    #     ##
    ...
    #     sum_tr14 = tr.rolling(window=14).sum()
    #     cond3 = ld > 0
    #     cond4 = ld > hd
    #     cond3[~cond4] = False
    #     data1 = ld
    ...
    #     sum1 = data1.rolling(window=14).sum()*100/sum_tr14
    #     cond5 = hd > 0
    #     cond6 = hd > ld
    #     cond5[~cond6] = False
    #     data2 = hd
    ...
    #     sum2= data2.rolling(window=14).sum()*100/sum_tr14
    #     #alpha = pd.rolling_mean((sum1 - sum2).abs() / (sum1 + sum2) * 100, 6).iloc[-1, :]
    #     alpha = ((sum1-sum2).abs()/(sum1+sum2)*100).rolling(window=6).iloc[-1,:]
    ...
    def alpha_172(self):
       ...
       # sum2 = pd.rolling_sum(data2, 14) * 100 / sum_tr14
        sum2= data2.rolling(window=14).sum()*100/sum_tr14
        #alpha = pd.rolling_mean((sum1 - sum2).abs() / (sum1 + sum2) * 100, 6).iloc[-1, :]
        alpha = ((sum1-sum2).abs()/(sum1+sum2)*100).rolling(window=6).mean()
        alpha = alpha.shift()
        return alpha.stack()
    # def alpha_173(self):
    #     ##
    ...
    #     data2= self.close.rolling(window=20).std()
    #     data2[~cond] = 0
    ...
    def alpha_174(self):
      ...
      #  data2 = pd.rolling_std(self.close, 20)
        ...
        data2[~cond] = 0
       # alpha = (pd.ewma(data2, span=39, adjust=False)).iloc[-1, :]
        alpha =data2.ewm(span=39,adjust=False).mean() 
        alpha = alpha.shift()
        return alpha.stack()
    # def alpha_175(self):
    #     ##
    ...
    def alpha_179(self):
        ...
        rank1=self.avg_price.rolling(window=4).corr(self.volume).rank(axis=0, pct=True)

        data2 = self.low.rank(axis=0, pct=True)
       
        data3 = self.volume.rolling(window=50).mean().rank(axis=0, pct=True)
       
        rank2=data2.rolling(window=12).corr(data3).rank(axis=0, pct=True)
        alpha = rank1 * rank2
        alpha = alpha.shift()
        return alpha.stack()
    # def alpha_180(self):
    ...
    def alpha_180(self):
        ...
        ma =self.volume.rolling(window=20).mean()
        cond = (ma < self.volume) 
        delta_close_7 = self.close.diff(7)
    ...
    #     tr = temp3  # MAX(MAX(HIGH-LOW,ABS(HIGH-DELAY(CLOSE,1))),ABS(LOW-DELAY(CLOSE,1)))
    #    # sum_tr14 = pd.rolling_sum(tr, 14)
    #     sum_tr14=tr.rollling(window=14).sum()
    #     cond3 = ld > 0
    #     cond4 = ld > hd
    #     cond3[~cond4] = False
    #     data1 = ld
    ...
    #     mean1 =((sum1-sum2).abs()/(sum1+sum2)*100).rolling(window=6).mean()
    ...
    def alpha_191(self):
      ...
      #  volume_avg = pd.rolling_mean(self.volume, window=20)
        ...
        corr=volume_avg.rolling(5).corr(self.low)
