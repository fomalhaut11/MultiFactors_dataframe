...
class GTJA_191:
 ...
 ##################################################################
    ...
    #     return alpha.dropna()
    def alpha_025(self, window=9, diffl=7):
        n = window
        part1 = self.close.diff(diffl).rank(axis=1, pct=True)
        temp = self.volume / self.volume.rolling(20).mean()
        seq = [2 * i / (n * (n + 1)) for i in range(1, n + 1)]
        weight = np.array(seq)
    ...
    def alpha_035(self, n=15, m=7, k=17):
        temp1 = self.open_price.diff()
        seq1 = [2 * i / (n * (n + 1)) for i in range(1, n + 1)]
        seq2 = [2 * i / (m * (m + 1)) for i in range(1, m + 1)]
        weight1 = np.array(seq1)
    ...
    #     alpha = result.iloc[-1, :]
    #     return alpha.dropna()
    def alpha_036(self, window=6, window2=2):
        temp1 = self.volume.rank(axis=1, pct=True)
        temp2 = self.avg_price.rank(axis=1, pct=True)
    ...
    def alpha_037(self, window=5, shiftl=10):
        ret = self.close.pct_change()
        temp = self.open_price.rolling(window).sum() * ret.rolling(window).sum()
        part1 = temp.rank(axis=1, pct=True)
        part2 = temp.shift(shiftl)
        result = -part1 - part2
        alpha = result.shift() 
    ...
    def alpha_039(
        ...
        temp2 = 0.3 * self.avg_price + 0.7 * self.open_price
        # volume_180 = pd.rolling_mean(self.volume, 180)
        volume_180 = self.volume.rolling(window).mean()
        # sum_vol = pd.rolling_sum(volume_180, 37)
    ...
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_041(self, diffn=3, maxn=5):
    ...
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_042(self, window1=10):
        part1 = self.high.rolling(window1).corr(self.volume)
    ...
    def alpha_044(
        ...
        weight1 = np.array(seq1)
        weight2 = np.array(seq2)
        part1 = temp1.rolling(p3).apply(lambda x: np.sum(x * weight1))
        part1 = part1.rolling(p5).apply(self.func_rank)
    ...
    def alpha_045(self, window1=150, window2=15):
        temp1 = self.close * 0.6 + self.open_price * 0.4
        part1 = temp1.diff()
        part1 = part1.rank(axis=1, pct=True)
        temp2 = self.volume.rolling(window1).mean()
    ...
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_061(self, n=12, m=17, window1=80, window2=8):
        n = 12
        m = 17
        temp1 = self.avg_price.diff()
        temp1 = temp1 
        ...
        temp2 = self.low
        temp2 = temp2.rolling(window2).corr(self.volume.rolling(window1).mean())
        temp2 = temp2.rank(axis=1, pct=1)
        part2 = temp2.rolling(m).apply(lambda x: np.sum(x * weight2))
        part2 = -part2.rank(axis=1, pct=1)
    ...
    def alpha_062(self, window=5):
        volume_rank = self.volume.rank(axis=1, pct=1)
        result = self.high.rolling(window).corr(volume_rank)
        alpha = -result.shift()
        return alpha.stack()
        ##################################################################
    ...
    def alpha_063(self, alphadown=6):
        part1 = np.maximum(self.close - self.close.shift(), 0)
        part1 = part1.ewm(alpha=1.0 / alphadown).mean()
        part2 = (self.close - self.close.shift()).abs()
    ...
    def alpha_066(self, window1=6):
        part1 = self.close.rolling(window1).mean()
    ...
    def alpha_067(self, alphadown=24):
        temp1 = self.close - self.close.shift()
        part1 = np.maximum(temp1, 0)
        part1 = part1.ewm(alpha=1.0 / alphadown).mean()
        temp2 = temp1.abs()
    ...
    #     return alpha
    def alpha_078(self, rolling_window=12, p1=0.015):
        # ((HIGH+LOW+CLOSE)/3-MA((HIGH+LOW+CLOSE)/3,12))/(0.015*MEAN(ABS(CLOSE-MEAN((HIGH+LOW+CLOSE)/3,12)),12))
        #
        # data1 = (self.high + self.low + self.close) / 3 - pd.rolling_mean((self.high + self.low + self.close) / 3,
        #                                                                   window=12)
    ...
    #     return alpha
    def alpha_086(self, shiftl1=10, shiftl2=20):
    ...
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_087(self, n=7, m=11, difflength=4):
        temp1 = self.avg_price.diff(difflength)
        seq1 = [2 * i / (n * (n + 1)) for i in range(1, n + 1)]
        seq2 = [2 * i / (m * (m + 1)) for i in range(1, m + 1)]
        weight1 = np.array(seq1)
        weight2 = np.array(seq2)
    ...
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_090(self, window1=5):
        # (rank(corr(rank(vwap),rank(volume),5))*-1)
        #######################
        data1 = self.avg_price.rank(axis=1, pct=True)
        data2 = self.volume.rank(axis=1, pct=True)
        corr = data1.rolling(window1).corr(data2)
        rank1 = corr.rank(pct=True)
        alpha = rank1 * -1
        alpha = alpha.shift()
        return alpha.stack()
    ...
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_091(self, window1=5, window2=40, window3=5):
        # ((rank((close-max(close,5)))*rank(corr((mean(volume,40)),low,5)))*-1)
        #################
    ...
    def alpha_092(self, window1=3, window2=180, window3=13, window4=5, window5=15):
        ...
        cor1 = cor1.fillna(0)
        data1 = cor1.rolling(window4).apply(self.func_decaylinear)
        rank2 = data1.rolling(window5).apply(lambda x: x.rank(pct=True)[-1])
    ...
    def alpha_095(self, window1=20):
        # STD(AMOUNT,20) #
        alpha = self.amount.rolling(window1).std()
        alpha = alpha.shift()
        return alpha.stack()
    ...
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_096(self, window1=9, window2=5):
        # SMA(SMA((CLOSE-TSMIN(LOW,9))/(TSMAX(HIGH,9)-TSMIN(LOW,9))*100,3,1),3,1) #
        # sma1 = pd.ewma(
        #     100 * (self.close - self.low.rolling(9).min()) / (self.high.rolling(9).max() - self.low.rolling(9).min()),
        #     span=5, adjust=False)
    ...
    def alpha_113(self, shift1=5, window1=5, window2=2, window3=20):
        # (-1*((rank((sum(delay(close,5),20)/20))*corr(close,volume,2))*rank(corr(sum(close,5),sum(close,20),2))))
        #####################
    ...
    def alpha_114(self, window=5, delay1=2):
        # ((rank(delay(((high-low)/(sum(close,5)/5)),2))*rank(rank(volume)))/(((high-low)/(sum(close,5)/5))/(vwap-close)))
        #####################
    ...
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_123(self, window1=20, window2=9, window3=6):
        #####((RANK(CORR(SUM(((HIGH+LOW)/2), 20), SUM(MEAN(VOLUME, 60), 20), 9)) < RANK(CORR(LOW, VOLUME, 6))) * -1)
        ##
        ...
        rank2 = corr2.rolling(window2).apply(lambda x: x.rank(axis=0, pct=True)[-1])
        rank1 = rank1.reindex(rank2.index).reindex(columns=rank2.columns)
        cond_max = rank1 > rank2
        rank2[cond_max] = rank1[cond_max]
        alpha = (rank1 < rank2) * (-1)
    ...
    #     return alpha
    def alpha_130(self, window1=40, window2=9, window3=7):
        #### alpha_130
        #### (RANK(DELCAYLINEAR(CORR(((HIGH + LOW) / 2), MEAN(VOLUME, 40), 9), 10)) / RANK(DELCAYLINEAR(CORR(RANK(VWAP), RANK(VOLUME), 7), 3)))
        ##
        data1 = (self.high + self.low) / 2
    ...
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_132(self, window=20):
        #### MEAN(AMOUNT, 20)
        ##
        alpha = self.amount.rolling(window).mean()
        alpha = alpha.shift()
        return alpha.stack()
    ...
    #     return alpha
    def alpha_138(self, window1=20, window2=8, window3=60):
        #### ((RANK(DECAYLINEAR(DELTA((((LOW * 0.7) + (VWAP * 0.3))), 3), 20)) - TSRANK(DECAYLINEAR(TSRANK(CORR(TSRANK(LOW, 8), TSRANK(MEAN(VOLUME, 60), 17), 5), 19), 16), 7)) * -1)
        ##
        data1 = (self.low * 0.7 + self.avg_price * 0.3).diff(3)
    ...
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_141(self, window1=15, window2=9):
        #### (RANK(CORR(RANK(HIGH), RANK(MEAN(VOLUME, 15)), 9))* -1)
        ##
        df1 = self.high.rank(axis=1, pct=True)
    ...
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_142(self, window1=10, window2=20, window3=5):
        #### (((-1 * RANK(TSRANK(CLOSE, 10))) * RANK(DELTA(DELTA(CLOSE, 1), 1))) * RANK(TSRANK((VOLUME/MEAN(VOLUME, 20)), 5)))
        ##
    ...
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_152(self, window1=9, span1=17):
        # SMA(MEAN(DELAY(SMA(DELAY(CLOSE/DELAY(CLOSE,9),1),9,1),1),12)-MEAN(DELAY(SMA(DELAY(CLOSE/DELAY(CLOSE,9),1),9,1),1),26),9,1) #
        #
        # data1 = pd.rolling_mean((pd.ewma(((self.close / self.close.shift(9)).shift()), span=17, adjust=False)).shift(),
        #                         12)
    ...
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_155(self, span1=13, span2=27, span3=10):
        # SMA(VOLUME,13,2)-SMA(VOLUME,27,2)-SMA(SMA(VOLUME,13,2)-SMA(VOLUME,27,2),10,2) #
        # sma1 = pd.ewma(self.volume, span=12, adjust=False)
        # sma2 = pd.ewma(self.volume, span=26, adjust=False)
        # sma = pd.ewma(sma1 - sma2, span=9, adjust=False)
    ...
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_162(self, span=23, c1=12):
        ###############(sma(max(close-delay(close,1),0),12,1)/sma(abs(close-delay(close,1)),12,1)*100-min(sma(max(close-delay(close,1),0),12,1)/sma(abs(close-delay(close,1)),12,1)*100,12))/(max(sma(max(close-delay(close,1),0),12,1)/sma(abs(close-delay(close,1)),12,1)*100),12)-min(sma(max(close-delay(close,1),0),12,1)/sma(abs(close-delay(close,1)),12,1)*100),12))
        #################
        # 算出公式核心部分X
        data1 = self.close - self.close.shift()
    ...
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_169(self, span1=17, span2=19, window1=12, window2=26):
        ##
        ###sma(mean(delay(sma(close-delay(close,1),9,1),1),12)-mean(delay(sma(close-delay(close,1),1,1),1),26),10,1)#####
        data1 = self.close - self.close.shift()
       # data2 = (pd.ewma(data1, span=17)).shift()
    ...
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_170(self, window1=20, window2=5):
        ##
        #####((((rank((1/close))*volume)/mean(volume,20))*((high*rank((high-close)))/(sum(high,5)/5)))-rank((vwap-delay(vwap,5))))####
        data1 = (1 / self.close).rank(axis=0, pct=True)
        #data2 = pd.rolling_mean(self.volume, 20)
    ...
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_172(self, window1=14, window2=6):
        # MEAN(ABS(SUM((LD>0&LD>HD)?LD:0,14)*100/SUM(TR,14)-SUM((HD>0&HD>LD)?HD:0,14)*100/(SUM((LD>0&LD>HD)?LD:0,14)*100/SUM(TR,14)+SUM(TR,14)+SUM((HD>0&HD>LD)?HD:0,14)*100/SUM(TR,14))*100,6) #
        hd = self.high - self.high.shift()
        ld = self.low.shift() - self.low
        temp1 = self.high - self.low
    ...
    #     alpha = alpha.dropna()
    #     return alpha
    def alpha_186(self, window1=14, window2=6):
        # (MEAN(ABS(SUM((LD>0 & LD>HD)?LD:0,14)*100/SUM(TR,14)-SUM((HD>0 & HD>LD)?HD:0,14)*100/SUM(TR,14))/(SUM((LD>0 & LD>HD)?LD:0,14)*100/SUM(TR,14)+SUM((HD>0 & HD>LD)?HD:0,14)*100/SUM(TR,14))*100,6)+DELAY(MEAN(ABS(SUM((LD>0 & LD>HD)?LD:0,14)*100/SUM(TR,14)-SUM((HD>0 & HD>LD)?HD:0,14)*100/SUM(TR,14))/(SUM((LD>0 & LD>HD)?LD:0,14)*100/SUM(TR,14)+SUM((HD>0 & HD>LD)?HD:0,14)*100/SUM(TR,14))*100,6),6))/2 #
        hd = self.high - self.high.shift()
        ld = self.low.shift() - self.low
        temp1 = self.high - self.low
