...
def Three_grouped_analysis(factordata, nextreturn):
    if "Factor_grouped" in factordata.columns:
        ...
        factordata["Factor_grouped"] = factordata.groupby("TradingDates")[
            "New_Factor"
        ].apply(lambda x: pd.qcut(x, 3, labels=False))
    if "Return_grouped" in nextreturn.columns:
        pass
    else:
        print("computing return grouped")
        nextreturn["Return_grouped"] = nextreturn.groupby("TradingDates")[
            "LogReturn"
        ].apply(lambda x: pd.qcut(x, 3, labels=False, duplicates="drop"))
    grouped_data = pd.merge(
        factordata["Factor_grouped"],
        nextreturn["Return_grouped"],
        on=["TradingDates", "StockCodes"],
    )
    grouped_data["correctidex"] = (
        grouped_data["Factor_grouped"] == grouped_data["Return_grouped"]
    )
    total_accuracy = grouped_data.groupby("Factor_grouped")["correctidex"].mean()
    daily_accuracy = grouped_data.groupby(["TradingDates", "Factor_grouped"])[
        "correctidex"
    ].mean()
...
def re_analysis(
    datapath=r"E:\Documents\PythonProject\StockProject\StockData\SingleFactorTestData02",
):
    LogReturn_daily_o2o = pd.read_pickle(
        r"E:\Documents\PythonProject\StockProject\StockData\LogReturn_daily_o2o.pkl"
    )
    LogReturn_daily_o2o["Return_grouped"] = LogReturn_daily_o2o.groupby(
        "TradingDates", group_keys=False
    )["LogReturn"].apply(lambda x: pd.qcut(x, 3, labels=False, duplicates="drop"))
    ...
    for file in files:
        ...
        data["Factor_grouped"] = data.groupby("TradingDates", group_keys=False)[
            "New_Factor"
        ].apply(lambda x: pd.qcut(x, 3, labels=False))
        total_accuracy, daily_accuracy = Three_grouped_analysis(
            data, LogReturn_daily_o2o
        )
        dailysum = daily_accuracy.groupby("TradingDates", group_keys=False).sum()
        all_daily_accuracy = pd.concat(
            [all_daily_accuracy, daily_accuracy.to_frame(name=file_name)], axis=1
        )
        all_daily_accuracy_sum = pd.concat(
            [all_daily_accuracy_sum, dailysum.to_frame(name=file_name)], axis=1
        )
...
def factor_timing(data, window_size=20) -> pd.DataFrame:
    ...
    result["PIR"] = (
        data["P_Regress"].rolling(window=window_size).mean()
        / data["P_Regress"].rolling(window=window_size).std()
    )
...
def loop_through_factor_timing(
    ...
    for file in files:
        ...
        print(file_name)
        result = factor_timing(data, window_size)
    ...
    filter1 = (P_ema1 < P_ema2) & (
        P_ema2 < 0.2
    )  # 选取P值指数平滑线性下降的因子且 P值小于0.2
    ...
    for i, begindate in enumerate(begindates):
        ...
        for factorname in regress_data_df.columns:
            if len(regressed_data) == 0:
                regressed_data = pd.to_numeric(
                    regress_data_df[factorname].dropna(), errors="coerce"
                ).to_frame(name=factorname)
                New_Factors = pd.to_numeric(
                    pridict_data_df[factorname].dropna(), errors="coerce"
                ).to_frame(name=factorname)
                continue
            tempdata = pd.to_numeric(
                regress_data_df[factorname].loc[regressed_data.index].fillna(0),
                errors="coerce",
            ).to_frame(name=factorname)
            tempNew_Factor = pd.to_numeric(
                pridict_data_df[factorname].loc[New_Factors.index].fillna(0),
                errors="coerce",
            ).to_frame(name=factorname)
            result = sm.OLS(tempdata, sm.add_constant(regressed_data)).fit()
            resid = result.resid
            tempdata = sft.Normlization(resid, method="zscore").to_frame(
                name=factorname
            )
            regressed_data = pd.concat([regressed_data, tempdata], axis=1)
            params0[factorname] = result.params
            tempNew_Factor = tempNew_Factor - (
                result.params[0] + result.params[1] * tempNew_Factor
            )  # 观测值-预测值
        ...
        New_Factors_weighted_sum = (
            (New_Factors.mul(weights_df.iloc[0]) / icirabssum)
            .sum(axis=1)
            .to_frame(name="CombinedFactor")
        )
...
def quick_test(factor, logreturn, single_base_data=None, groupnums=10):
    ...
    for date in tradingdates:
        ...
        if single_base_data is not None:
            ...
            resid = model.resid
            newfactor = sft.Normlization(resid, method="zscore").to_frame(
                name="Newfactor"
            )
            x1 = X.join(newfactor)
        else:
        ...
        analysis_data.loc[date, "Factor_Return_Regress"] = result1.params["newfactor"]
        analysis_data.loc[date, "T_Regress"] = result1.tvalues["newfactor"]
        analysis_data.loc[date, "P_Regress"] = result1.pvalues["newfactor"]
        analysis_data.loc[date, "IC_Regress"] = spearmanr(newfactor, resid_return)[0]
        """收益率分组测试 """
        ...
        Grouped_StockCodes["ReturnMean"] = pd.to_numeric(
            Grouped_StockCodes["ReturnMean"], errors="coerce"
        )
        grouped_cumsum = (
            Grouped_StockCodes["ReturnMean"]
            .groupby(Grouped_StockCodes.index.get_level_values("Group"))
            .cumsum()
        )
...
class LoopThroughSingleFactorTest:
    ...
    def __init__(self, single_test) -> None:
        ...
        self.recordinfopath = (
            r"E:\Documents\PythonProject\StockProject\StockData\multifactorrecord"
        )
    ...
    def single_test_run(self, datasavepath) -> None:
        ...
        for factorfile in self.factorsfiles:
            ...
            self.single_test.data_plot(
                meanlen=120,
                AbsReturn=False,
                savepath=os.path.join(datasavepath, "graphic"),
            )
            self.single_test.data_save(
                os.path.join(datasavepath, "newfactors"),
...
def run_multifactors_Main(input_data_path):
    ...
    factor_exposure_benchmark_ = Calc_factors_exposure(StockHoldings, basefactors)

    pass


def renew_loopthrough_singlefactor_test(
    singletestpath,
    rowfactors_rela_path=r"RawFactors",
    basepath=r"E:\Documents\PythonProject\StockProject\StockData",
) -> None:
    """更新单因子测试：单因子测试配置文件在singletestpath下, 价格数据在basepath下"""
    inipath = os.path.join(singletestpath, "[SingleFactorTest].ini")
    single_test = sft.Single_Factor_Test(inipath)
    pricedfpath = os.path.join(basepath, "Price.pkl")
    PriceDf = pd.read_pickle(pricedfpath)
    single_test.stock_filter(PriceDf)
    single_test.data_loading_1st_time(c_type="one_hot")
    datasavepath = singletestpath
    Model1 = LoopThroughSingleFactorTest(single_test)
    Rawfactorspath = os.path.join(basepath, rowfactors_rela_path)
    Model1.single_test.rawfactorsavepath = Rawfactorspath
    Model1.single_test_run(datasavepath)


if __name__ == "__main__":
    print("ready")
    hs300w = SDP.WideBaseComponentFromSql("SH000300")
    renew_loopthrough_singlefactor_test(
        r"E:\Documents\PythonProject\StockProject\StockData\SingleFactorTestData"
    )

#   picked_factor = loop_through_factor_timing(window_size=250, datapath=r'E:\Documents\PythonProject\StockProject\StockData\SingleFactorTestData')
