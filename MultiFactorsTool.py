...
def FactorCombining(
    pickfacorlist,
    datasavepath=r"E:\Documents\PythonProject\StockProject\StockData\TestTempData",
):
    for i in range(len(pickfactor)):
        ...
        data1 = (
            data["newfactor"]
            .to_frame(name=pickfactor[i])
            .reset_index(level=0, drop=True)
            .sort_index(level=0)
        )
        data2 = (
            data["T"].squeeze().sort_index(level=0).shift(1)
        )  # T是计算下期收益和当期因子值的 ，所以要shift一期
        ...
        if i == 0:
            ...
            residuals = df.groupby(level=0).apply(
                lambda x: sm.OLS(x["data1"], sm.add_constant(x.filter(like="_factor")))
                .fit()
                .resid
            )
...
def FactorCombiningAnalysis(
    ...
    for i in range(len(pickfactor)):
        ...
        if i == 0:
            ...
            residuals = residuals.reset_index(level=0, drop=True)
            reiduals.groupby(level=0).apply(
                lambda x: sm.OLS(sm.add_constant(stockreturndata), x)
                .fit()
                .tvalues["newfactor"]
            )
            factor[pickfactor[i]] = residuals.loc[factor.index] * data2sign
...
def weight_linear_program_numpy(
    factors_explosure, benchmark_explosure, constraints, logreturn
):
...
if __name__ == "__main__":
    ...
    next_log_return = pd.read_pickle(
        os.path.join(datasavepath, "LogReturn_daily_o2o.pkl")
    )
    ...
    pd.to_pickle(
        ...
        os.path.join(
            datasavepath,
            "BP_t_profitgrowth_zz2000_20day_beta_spg_log3marketcap_SUE_ss_4_hd5_alpha001_alpha032.pkl",
        ),
    )
    test.data_backtest_one_hot(
        "BP_t_profitgrowth_zz2000_20day_beta_spg_log3marketcap_SUE_ss_4_hd5_alpha001_alpha032"
    )
