...
def cal_IC_P_GroupReturn_inday(newfactor, next_log_return, groupnum=10):
    ...
    sorted_return_group = np.array_split(
        next_log_return[sort_indice], groupnum)
    ...
    return (IC, P, mean_return_group,
            std_return_group, mean_factor_group, sorted_indice_group)


def PickupStocksByAmount(PriceDF: pd.DataFrame,
                         windows=20,
                         para=0,
                         mc_limit_min=None,
                         mc_limit_max=None) -> pd.DataFrame:

    """过去5天平均成交额大于para,市值>mclimit,amt>0,l<h,的股票"""
    average_amount = PriceDF.groupby('StockCodes')['amt']. \
        rolling(window=windows).mean().reset_index(level=0, drop=True).shift(1)
    ...
    filtered_stocks = filtered_stocks[PriceDF['o'] <
                                      PriceDF['high_limit'] - 0.02]  # 剔除开盘涨停股票
    filtered_stocks = filtered_stocks[PriceDF['o'] >
                                      PriceDF['low_limit'] + 0.02]  # 剔除开盘跌停股票
    # 找出所有股票代码的第一个数字小于7的所有数据, 剔除北交所
    filtered_stocks = filtered_stocks[
        filtered_stocks.index.get_level_values('StockCodes').
        str[0].astype(int) < 7]
    filtered_stocks = filtered_stocks.to_frame().sort_index(level=0)
    return filtered_stocks


def PickupStocksByMarketCap(PriceDF: pd.DataFrame,
                            groupnums=10,
                            targetgroup=0) -> pd.DataFrame:
    """按市值分组，选取市值最小的股票"""
    filtered_stocks = PriceDF
    # 找出所有股票代码的第一个数字小于7的所有数据, 剔除北交所
    filtered_stocks = filtered_stocks[
        filtered_stocks.index.
        get_level_values('StockCodes').str[0].astype(int) < 7]
...
def backtest_merged_data(
                merged_data,
                groupnums,
                IndusNetralBase=True,
                extradates=0):
    ...
    columns_to_exclude = base_factor_columns.tolist() + \
        ['Factor'] + ['LogReturn']
    ...
    Resid_Return = pd.DataFrame(index=merged_data.index,
                                columns=['Resid_Return'])  # 残差收益率
    """输出数据初始化"""
    ...
    for date in tradingdates:
        ...
        if y0.std().values[0] <= 0.00001:  # 如果因子是布尔值(0,1)离散型，则不做去极值，只做标准化
        ...
        if len(base_factor_columns) > 0:
            ...
            if len(industry_columns) > 0:
                one_hot = m1slice[industry_columns]. \
                    dropna(axis=1, how='all').fillna(0)
                if IndusNetralBase:
                    for j in range(basesize):
                        result_base = sm.OLS(
                            base_factors.iloc[:, j],
                            one_hot.loc[base_factors.index]).fit()
                        temp_base = result_base.resid
                        temp_base = Remove_Outlier(
                            temp_base, method='IQR', para=3)
                ...
                X = base_factors.join(one_hot.loc[common_index])  # 基准因子+行业独热编码
...
def backtest_nearestmatrix_np(inputdata, NetralBase=True):
    ...
    backtestindex = np.where(
            ...
            (inputdata_ns.merged_data_tradingdates <=
                pd.to_datetime(inputdata_ns.EndDate, format='%Y-%m-%d')))[0]

    """输出数据初始化"""
    tradingdates = inputdata_ns.merged_data_tradingdates[backtestindex]
    New_Factor_numpy = np.zeros(
        (len(tradingdates), len(inputdata_ns.StockCodes_unstacked)))*np.nan
    standared_basefactor_numpy = np.zeros(
                        (len(tradingdates),
                            len(inputdata_ns.StockCodes_unstacked),
                            len(inputdata_ns.base_data.columns))
                                            )*np.nan

    Resid_Return_numpy = np.zeros(
        (len(tradingdates),
            len(inputdata_ns.StockCodes_unstacked)))*np.nan
    paramsdata_numpy = np.zeros(
        (len(tradingdates),
            len(inputdata_ns.base_data.columns)+1))*np.nan
    pvalues_numpy = np.zeros(
        (len(tradingdates),
            len(inputdata_ns.base_data.columns)+1))*np.nan
    tvalues_numpy = np.zeros(
        (len(tradingdates),
            len(inputdata_ns.base_data.columns)+1))*np.nan
    ...
    analysis_data_numpy = np.zeros(
        (len(tradingdates),
            len(analysis_data_columns)))*np.nan
    groups = np.arange(groupnums, dtype=np.int64)
    Grouped_index = pd.MultiIndex.from_product(
                    [tradingdates, groups],
                    names=['TradingDates', 'Group'])
    ...
    for i in range(0, len(backtestindex)):
        ...
        today_stock_codes = \
            inputdata_ns.Intersect_StockCodes[
                inputdata_ns.
                Intersect_StockCodes_indice_StockCodesUnstacked
                [factor_nonan_indice]]
        # 当前日相似度矩阵
        ...
        for j in range(len(factor_nonan_indice)):
            ...
            indice1 = \
                inputdata_ns.StockCodes_Unstacked_indice_of_Nearest[Nindice]
            ...
            if len(np.where(~np.isnan(Factor_j_Nearest))[0]) <= \
                ...
                today_newfactor0[j] = (
                    (factor[j]-np.mean(Factor_j_Nearest)) /
        ...
        standared_basefactor_numpy[
            i,
            inputdata_ns.Intersect_StockCodes_indice_StockCodesUnstacked[
                factor_nonan_indice], :] = today_newBaseFactor1.T
        New_Factor_numpy[
            i,
            inputdata_ns.Intersect_StockCodes_indice_StockCodesUnstacked[
                factor_nonan_indice]] = today_newfactor
        """因子收益率回归测试"""
        X2 = sm.add_constant(np.concatenate(
            [today_newBaseFactor1.T,
                today_newfactor.reshape(-1, 1)], axis=1))
        ...
        Resid_Return_numpy[
            i,
            inputdata_ns.Intersect_StockCodes_indice_StockCodesUnstacked[
                factor_nonan_indice]] = reisd_return
        ...
        analysis_data_numpy[i, 4] = spearmanr(
                today_newfactor, nextlogreturn)[0]
        ...
        for g in range(groupnums):
            ...
            end_idx = (g + 1) * target_size \
                if g < groupnums - 1 else len(today_newfactor)
            ...
            grouped_factor_mean[g] = \
                sorted_newfactor[start_idx:end_idx].mean()
        ...
        analysis_data_numpy[i, 5] = spearmanr(
            grouped_factor_mean, grouped_means)[0]
        analysis_data_numpy[i, 7] = spearmanr(
            grouped_factor_mean, grouped_means)[1]
...
class Single_Factor_Test():
    def __init__(self, config_file):
        ...
        self.BeginDate = config.get(
            'SingleFactorTest', 'BeginDate')  # 回测开始日期
        self.EndDate = config.get(
            'SingleFactorTest', 'EndDate')  # 回测结束日期
        self.EndDate_tag = 'default'  # 如果使用default，则默认测试结束日期为因子的最新日期
        self.backtesttype = config.get(
            'SingleFactorTest', 'backtesttype')  # 回测交易频率 daily weekly monthly
        self.basic_data_path = config.get(
            'SingleFactorTest', 'basic_data_path')  # 基础数据存储路径
        self.rawfactorsavepath = config.get(
            'SingleFactorTest', 'raw_factor_path')  # 原始因子数据存储路径
        self.basedatasavepath = config.get(
            'SingleFactorTest', 'base_data_path')  # 基准数据存储路径
        self.tempdatapath = config.get(
            'SingleFactorTest', 'temp_data_path')  # 临时数据存储路径
        self.base_name = config.get(
            'SingleFactorTest', 'base_name').split(',')  # 基准名称
        self.matrixname = config.get(
            'SingleFactorTest', 'matrixname')  # 相似度矩阵名称
        self.nearestnums = config.getint(
            'SingleFactorTest', 'nearestnums')  # 最近邻个数
        self.c_name = config.get(
            'SingleFactorTest', 'c_name')  # 独热分类器名称
        self.groupnums = config.getint(
            'SingleFactorTest', 'groupnums')  # 回测分组数量
        self.NetralBase = config.getboolean(
            'SingleFactorTest', 'NetralBase')  # 是否中性化基准
        self.backtesttradingprice = config.get(
            'SingleFactorTest', 'back_test_trading_price')  # 回测交易价格#vwap o2o
    ...
    def data_merging(self) -> None:
        ...
        factor_data = (
            self.factor_data
            .reindex(self.filtered_stocks.index)
            .groupby(level=1).fillna(0)
                      )
        next_log_return = (
            self.next_log_return
            .reindex(self.filtered_stocks.index)
            .groupby(level=1).fillna(0)
                            )
        ...
        if len(self.base_name) > 0:
            if hasattr(self, 'base_data'):
                base_data = (
                    ...
                    .groupby(level=1).fillna(method='ffill')
                            )
                merged_data = pd.concat(
                    [merged_data, base_data], axis=1)
            else:
        ...
        if self.c_name == 'classification_one_hot':
            if hasattr(self, 'classification_data'):
                ...
                merged_data = pd.concat(
                    [merged_data, classification_data], axis=1)
            else:
    ...
    def data_loading_1st_time(self, c_type) -> None:
        ...
        if self.backtesttradingprice == 'o2o':
            if self.backtesttype == 'daily':
                self.next_log_return = pd.read_pickle(
                    os.path.join(
                        self.basic_data_path, 'LogReturn_daily_o2o.pkl'
                                ))
            if self.backtesttype == 'weekly':
                self.next_log_return = pd.read_pickle(
                    os.path.join(
                        self.basic_data_path, 'LogReturn_weekly_o2o.pkl'
                                ))
            if self.backtesttype == 'monthly':
                self.next_log_return = pd.read_pickle(
                    os.path.join(
                        self.basic_data_path, 'LogReturn_monthly_o2o.pkl'
                                ))
        """读取下期收益数据"""
        if self.backtesttradingprice == 'vwap':
            if self.backtesttype == 'daily':
                self.next_log_return = pd.read_pickle(
                    os.path.join(
                        self.basic_data_path, 'LogReturn_daily_vwap.pkl'
                        ))
            if self.backtesttype == 'weekly':
                self.next_log_return = pd.read_pickle(
                    os.path.join(
                        self.basic_data_path, 'LogReturn_weekly_vwap.pkl'))
            if self.backtesttype == 'monthly':
                self.next_log_return = pd.read_pickle(
                    os.path.join(
                        self.basic_data_path, 'LogReturn_monthly_vwap.pkl'))
        self.next_log_return = self.next_log_return.sort_index(level=0)
        # 读取下期收益数据  本日时间戳买进，下个时间戳卖出的收益
        """读取回测日期"""
        self.backtestdates = (
            self.next_log_return.index
            .get_level_values(0)
            .unique()
            .sort_values()
                                )
        ...
        if isinstance(self.base_name, list):  # 考虑基准因子为多个的情况
            ...
            for i in range(0, len(self.base_name)):
                temp = pd.read_pickle(
                    os.path.join(
                        self.rawfactorsavepath, self.base_name[i]+'.pkl')
                                        )
                if i == 0:
                    base_data = pd.DataFrame()
        ...
        if (c_type == 'one_hot') | (c_type == 'classification_one_hot'):
            ...
            # 去除重复的行
            self.classification_data = \
                self.classification_data.drop_duplicates(
                    subset=['TradingDates', 'StockCodes'])
            # 将索引转换回来
            self.classification_data = \
                self.classification_data.set_index(
            ...
            if hasattr(self, 'filtered_stocks'):
                ...
                self.merged_data = pd.concat(
                    [base_data, next_log_return], axis=1)
                duplicated_indices = self.merged_data.index.duplicated()
                self.merged_data = self.merged_data[~duplicated_indices]
    ...
    def data_loading_factor(self, factor_name, factorspath=None) -> None:
        ...
        if isinstance(self.factor_data, pd.Series):
            self.factor_data = (
                ...
                to_frame().rename(columns={0: 'Factor'})
                                )
    ...
    def data_reloading_base(self, newbasename):
        ...
        # 删除原有basefactor
        columns_to_drop = [
            col for col in self.base_data.columns if 'BaseFactor' in col]
        ...
        if isinstance(newbasename, list):
            for i in range(0, len(newbasename)):
                temp = (
                    pd.read_pickle(
                        os.path.join(
                            self.rawfactorsavepath, newbasename[i]+'.pkl'
                                        )
                                    )
    ...
    def backtest_pandas(self,
        ...
        factorbegindate = self.factor_data.index.\
            get_level_values(0).unique().sort_values()[0]
        factorenddate = self.factor_data.index.\
            get_level_values(0).unique().sort_values()[-1]
        begindate = max(
            pd.to_datetime(self.BeginDate, format='%Y-%m-%d'),
            factorbegindate)
        ...
        analysis_data, New_Factor, Resid_Return, Grouped_StockCodes = \
            backtest_merged_data(
                m1, self.groupnums, IndusNetralBase=True, extradates=0)

        return analysis_data, New_Factor, Resid_Return, Grouped_StockCodes

    def backtest_one_hot_cat_pandas(
            self, factor_name,
            groupnums=None, NetralBase=False, factorspath=None
            ):
        ...
        if hasattr(self, 'merged_data'):
            ...
            print('ReloadingData')
            self.data_loading_1st_time()
        factorbegindate = self.factor_data.index.get_level_values(0).\
            unique().sort_values()[0]
        factorenddate = self.factor_data.index.get_level_values(0).\
            unique().sort_values()[-1]
        begindate = max(
            pd.to_datetime(self.BeginDate, format='%Y-%m-%d'),
            factorbegindate)
        if self.EndDate_tag == 'EndDate':
        ...
        for date in tradingdates:
            ...
            if (y2 == 0).all().all():  # 当日因子值全为0，不需要进行回归分析
                ...
                base_factors = m1slice[
                    m1slice.filter(like='BaseFactor').columns].copy()
                basesize = len(base_factors.columns)
                for j in range(basesize):
                    base_factors.iloc[:, j] = Remove_Outlier(
                        base_factors.iloc[:, j],
                ...
                industry_columns = m1slice.drop(
                    columns=base_factors.columns.tolist() +
                    ['Factor', 'LogReturn']
                    ).dropna(axis=1, how='all')  # 行业独热编码,剔除在所有分类上都为nan的行
                ...
                analysis_data.loc[date, 'Factor_Return_LongShort'] = \
                    ...
                    groupedmean.loc[min(groupedmean.index)]
                groupedfactormean = \
                    m1slice.groupby('Group')['newfactor'].mean()
                correlation, p_value = spearmanr(
                    groupedfactormean, groupedmean)
                analysis_data.loc[date, 'IC_Grouped'] = correlation
                analysis_data.loc[date, 'P_Grouped'] = p_value
                ...
                groupedmean.index = pd.MultiIndex.from_product(
                    [[date], groups], names=['TradingDates', 'Group'])
                Grouped_StockCodes.loc[
                    groupedmean.index, 'ReturnMean'] = groupedmean
                groupedstd = grouped['LogReturn'].std()
                groupedstd.index = pd.MultiIndex.from_product(
                    [[date], groups], names=['TradingDates', 'Group'])
                Grouped_StockCodes.loc[
                    groupedstd.index, 'ReturnStd'] = groupedstd
                groupedcodes = m1slice.groupby('Group').apply(
                    lambda x: x.index.get_level_values('StockCodes').tolist())
                groupedcodes.index = pd.MultiIndex.from_product(
                    [[date], groups], names=['TradingDates', 'Group'])
                Grouped_StockCodes.loc[
                    groupedcodes.index, 'StockCodes'] = groupedcodes

                newfactor.index = pd.MultiIndex.from_product(
                    [[date], newfactor.index],
                    names=['TradingDates', 'StockCodes'])
                New_Factor.loc[
                    newfactor.index, 'New_Factor'] = newfactor
                resid_return.index = pd.MultiIndex.from_product(
                    [[date], resid_return.index],
                    names=['TradingDates', 'StockCodes'])
                Resid_Return.loc[
                    resid_return.index, 'Resid_Return'] = resid_return.values
    ...
    def data_preprocessing_numpy(
                            self,
                            nearestdata_path=None,
                            nearestMatrixname=None,
                            nearestnums=None
                            ) -> None:
        """使用最近邻矩阵的数据的预处理"""
        assert hasattr(self, 'merged_data'), 'merged_data not loaded'
        ...
        if nearestdata_path is None:
            nearestdata_path = \
                os.path.join(self.basic_data_path, 'Classificationdata')
        ...
        self.StockCodes_unstacked = (
            merge_data_unstacked['Factor'].columns.to_numpy()
                                    )
        ...
        if hasattr(self, 'NearestIndice'):
            ...
            self.NearestIndice = \
                NearestIndice['NearestIndiceMatrix'][:, :, -nearestnums:]
            self.NearestTradingDates = NearestIndice['TradingDates']
            self.NearestTradingDates = \
                pd.to_datetime(self.NearestTradingDates, format='%Y%m%d')
            self.NearestStockCodes = NearestIndice['StockCodes']
            stock1, stockindice1, stockindice2 = \
                np.intersect1d(self.StockCodes_unstacked,
                               self.NearestStockCodes,
                               return_indices=True)
            ...
            self.StockCodes_Unstacked_indice_of_Nearest = \
                np.array(StockCode_in_Nearest_indice)
    ...
    def backtest_nearest_numpy(self, NetralBase='True'):
        ...
        New_Factor_numpy = np.zeros(
            (len(tradingdates), len(self.StockCodes_unstacked)))*np.nan
        standared_basefactor_numpy = np.zeros(
                            (len(tradingdates),
        ...
        Resid_Return_numpy = np.zeros(
            (len(tradingdates),
             len(self.StockCodes_unstacked)))*np.nan
        paramsdata_numpy = np.zeros(
            (len(tradingdates),
             len(self.base_data.columns)+1))*np.nan
        pvalues_numpy = np.zeros(
            (len(tradingdates),
             len(self.base_data.columns)+1))*np.nan
        tvalues_numpy = np.zeros(
        ...
        for i in range(0, len(backtestindex)):
            ...
            nearest = self.NearestIndice[
                date_idx_nearestmatrix,
                self.Intersect_StockCodes_indice_NearestStockCodes[
                                                factor_nonan_indice], :
                                        ]
            # 取出前一日 所有股票池 与相似度矩阵股票交集 最相近的股票索引
        ...
        New_Factor = pd.DataFrame(
            New_Factor_numpy,
            index=tradingdates,
            columns=self.StockCodes_unstacked)
        Resid_Return = pd.DataFrame(
            Resid_Return_numpy,
            index=tradingdates,
            columns=self.StockCodes_unstacked)
        Grouped_StockCodes = pd.DataFrame(
            Grouped_StockCodes_numpy.reshape(
                len(tradingdates)*len(groups), 3),
            index=Grouped_index,
            columns=Grouped_columns)
        paramsdata_pd = pd.DataFrame(
    ...
    def data_save(
        ...
        pd.to_pickle(self.New_Factor, os.path.join(newfactorsavepath, self.factor_name + '_newfactor.pkl'))
        pd.to_pickle(self.analysis_data, os.path.join(analysisdatasavepath, self.factor_name+'_analysis_data.pkl'))
        pd.to_pickle(self.Resid_Return, os.path.join(resid_return_savepath, self.factor_name+'_Resid_Return.pkl'))
    ...
    def data_plot(self, meanlen=120, AbsReturn=False, savepath=None):
        ...
        if AbsReturn is False:
            ...
            for i in range(0, self.groupnums):
                axs[0].plot(
                    self.Grouped_StockCodes.xs(i, level=1)['ReturnMean']
                    .cumsum(), label='Group'+str(i))
            axs[0].legend(loc='upper left')
            ax2 = axs[1].twinx()
            line1 = self.analysis_data['T_Regress'].dropna().\
                rolling(meanlen).mean().plot(ax=ax2, color='b', label=None)
            # 使用第二个 y 轴绘制 self.T
            line2 = self.analysis_data['IC_Grouped'].dropna().\
                rolling(meanlen).mean().plot(ax=axs[1], color='r', label=None)
            # 使用原始 y 轴绘制 self.IC
        ...
        # 如果需要，你还可以设置每个 y 轴的标签
            ...
            meanreturn = self.Grouped_StockCodes.\
                groupby(level=0)['ReturnMean'].mean()
            relative_meanreturn = self.Grouped_StockCodes['ReturnMean'] - \
                meanreturn
            fig, axs = plt.subplots(2, gridspec_kw={'height_ratios': [4, 1]})
            for i in range(0, self.groupnums):
                axs[0].plot(
                    relative_meanreturn.xs(i, level=1).cumsum(),
                    label='Group' + str(i))
            axs[0].legend(loc='upper left')
            ax2 = axs[1].twinx()
            line1 = self.analysis_data['T_Regress'].dropna().rolling(meanlen).mean().plot(ax=ax2, color='b', label=None)  # 使用第二个 y 轴绘制 self.T
            line2 = self.analysis_data['IC_Grouped'].dropna().rolling(meanlen).mean().plot(ax=axs[1], color='r', label=None)  # 使用原始 y 轴绘制 self.IC  
            ax2.yaxis.label.set_color('b')  # 设置第二个 y 轴的标签颜色
            axs[1].yaxis.label.set_color('r')  # 设置原始 y 轴的标签颜色
        # 如果需要，你还可以设置每个 y 轴的标签
            ax2.set_ylabel('T_Regress')
...
def NearestMatrixSingleFactorTest():
    test = Single_Factor_Test(r'E:\Documents\PythonProject\StockProject\StockData\SingleFactorTestData03\[SingleFactorTest].ini')  # 读取配置文件
    ...
    test.data_loading_factor('PEG', r'E:\Documents\PythonProject\StockProject\StockData\RawFactors')  
...
if __name__ == '__main__':
    ...
    analysis_data, New_Factor, Resid_Return, Grouped_StockCodes, \
        paramsdata_list, pvalues_list, tvalues_list = \
        test.backtest_nearest_numpy(NetralBase=True)

    test.data_plot(meanlen=120, AbsReturn=False, savepath=r'E:\Documents\PythonProject\StockProject\StockData\SingleFactorTestData\graphic')
    test.data_save(r'E:\Documents\PythonProject\StockProject\StockData\SingleFactorTestData\newfactors', 
