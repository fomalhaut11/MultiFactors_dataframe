...
import numpy as np
import pickle
files = glob.glob('E:\Documents\PythonProject\StockProject\StockData\TestTempData\*data.pkl')
...
def select_data_negative(data1, rolling_window=120, icband=0.01):
    ...
    for date in TradingDates:
        ...
        if (ic < -icband).any():
            max_group = data1['grouped_StockCodes'].loc[date].index.get_level_values('Group').max()
            negative_holdings[date] = data1['grouped_StockCodes'].loc[date].loc[max_group]
        elif (ic > icband).any():
            min_group = data1['grouped_StockCodes'].loc[date].index.get_level_values('Group').min()
            negative_holdings[date] = data1['grouped_StockCodes'].loc[date].loc[min_group]
        elif ic.isna().any():
...
def select_data_passitive(data1, rolling_window=120, icband=0.01):
    ...
    for date in TradingDates:
        ...
        if (ic < -icband).any():
            max_group = data1['grouped_StockCodes'].loc[date].index.get_level_values('Group').min()
            negative_holdings[date] = data1['grouped_StockCodes'].loc[date].loc[max_group]
        elif (ic > icband).any():
            min_group = data1['grouped_StockCodes'].loc[date].index.get_level_values('Group').max()
            negative_holdings[date] = data1['grouped_StockCodes'].loc[date].loc[min_group]
        elif ic.isna().any():
            negative_holdings[date] = []
        else:    
...
def non_negative_return(LogReturn, negative_holdings):
    ...
    for date in negative_holdings.index.get_level_values(0).unique():
        ...
        selected_stocks = [stock for stock in stocks_today if stock not in negative_holdings_today]
...
for file in files:
    ...
    if count == 0:
        ...
        negative_holdings = negative_holdings.combine(negative_holdings1, func=union_lists)
    # if count == 3:
    #     break
    count += 1    
    print(count)
...
for file in files:
    ...
    if count == 0:
        ...
        passitive_holdings = passitive_holdings.combine(passitive_holdings1, func=intersect_lists)
    # if count == 3:
    #     break
    count += 1    
    print(count)  
...
diffdata=(result1.groupby('TradingDates').mean()-LogReturn.groupby('TradingDates').mean()) 
