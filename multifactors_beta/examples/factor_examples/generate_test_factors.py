...
def load_auxiliary_data():
    ...
    # 加载市值数据
    market_cap_file = Path(get_config('main.paths.data_root')) / 'MarketCap.pkl'
    if market_cap_file.exists():
        ...
        # 尝试其他可能的路径
        alt_paths = [
            Path(get_config('main.paths.data_root')) / 'LogMarketCap.pkl',
    ...
    if trading_dates_file.exists():
        ...
        # 尝试从StockData路径加载
        alt_path = Path(get_config('main.paths.data_root')) / 'TradingDates.pkl'
        if alt_path.exists():
