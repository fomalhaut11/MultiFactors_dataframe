...
class EvaluationResult:
    ...
    # 维度详细结果
    dimension_results: Dict[str, DimensionScore] = field(default_factory=dict)
...
class FactorEvaluator(AnalyzerBase):
    ...
    def _initialize_dimensions(self) -> Dict[str, BaseDimension]:
        ...
        """
        # 根据场景获取维度权重
        dimension_weights = self._get_scenario_weights()
        
        # 初始化各维度评估器
        dimensions = {
            'profitability': ProfitabilityDimension(
                weight=dimension_weights.get('profitability', 0.35),
                config=self.config
            ),
            'stability': StabilityDimension(
                weight=dimension_weights.get('stability', 0.25),
                config=self.config
            ),
            'tradability': TradabilityDimension(
                weight=dimension_weights.get('tradability', 0.20),
                config=self.config
            ),
            'uniqueness': UniquenessDimension(
                weight=dimension_weights.get('uniqueness', 0.10),
                config=self.config
            ),
            'timeliness': TimelinesDimension(
                weight=dimension_weights.get('timeliness', 0.10),
                config=self.config
            )
        }
        
        logger.info(f"Dimensions initialized for scenario '{self.scenario}'")
        return dimensions
    
    def _get_scenario_weights(self) -> Dict[str, float]:
        """
        根据场景获取维度权重
        ...
        scenario_weights = {
            ...
            'high_frequency': {
                'profitability': 0.25,
                'stability': 0.15,
                'tradability': 0.35,  # 高频更重视可交易性
                'uniqueness': 0.10,
                'timeliness': 0.15    # 高频更重视时效性
            },
            'value_investing': {
                'profitability': 0.40,  # 价值投资更重视收益
                'stability': 0.30,      # 价值投资更重视稳定性
                'tradability': 0.10,
                'uniqueness': 0.10,
                'timeliness': 0.10
            },
        ...
        return scenario_weights.get(self.scenario, scenario_weights['balanced'])
    ...
    def evaluate(self,
        ...
        **kwargs : dict
        ...
        evaluation_result = EvaluationResult(
            ...
            dimension_results=self._dimension_results,  # 添加维度详细结果
    ...
    def _calculate_dimension_scores(self, data: Dict[str, Any]) -> Dict[str, float]:
        ...
        eval_data = {
            ...
            'correlation_analysis': data.get('correlation_result'),
            'stability_analysis': data.get('stability_result'),
            'factor_name': data.get('test_result').factor_name if data.get('test_result') else 'unknown'
        }
        ...
        for dim_name, dimension in self.dimensions.items():
            try:
                # 计算维度得分
                dimension_result = dimension.calculate_score(eval_data)
                
                # 保存得分和详细结果
                dimension_scores[dim_name] = dimension_result.score
                dimension_results[dim_name] = dimension_result
                
                logger.debug(f"{dim_name} dimension score: {dimension_result.score:.1f} ({dimension_result.grade})")
            except Exception as e:
                logger.warning(f"Failed to calculate {dim_name} dimension: {e}")
                # 使用默认得分
    ...
    def _generate_diagnostics(self,
        ...
        # 从维度结果中提取详细诊断信息
        if hasattr(self, '_dimension_results'):
            for dim_name, dim_result in self._dimension_results.items():
                # 使用维度的等级信息
                if dim_result.grade in ['AAA', 'AA', 'A']:
                    diagnostics['strengths'].append(
                        f"{dim_name}维度表现优秀({dim_result.score:.1f}分, {dim_result.grade}级)"
                    )
                elif dim_result.grade in ['D', 'F']:
                    diagnostics['weaknesses'].append(
                        f"{dim_name}维度需要改进({dim_result.score:.1f}分, {dim_result.grade}级)"
                    )
        else:
            # 后备逻辑
    ...
    def generate_evaluation_report(self, 
        ...
        for dim, score in evaluation_result.dimension_scores.items():
            # 获取维度等级
            dim_grade = ''
            if evaluation_result.dimension_results and dim in evaluation_result.dimension_results:
                dim_grade = f" ({evaluation_result.dimension_results[dim].grade}级)"
            lines.append(f"  {dim:15s}: {score:6.1f}分{dim_grade}")
    ...
    def get_dimension_details(self, factor_name: str, dimension: str) -> Optional[DimensionScore]:
        """
        获取特定维度的详细评估结果
        ...
        Returns
        -------
        DimensionScore or None
            维度评估详细结果
        """
        if factor_name in self.evaluation_results:
            result = self.evaluation_results[factor_name]
            if result.dimension_results and dimension in result.dimension_results:
                return result.dimension_results[dimension]
        return None