...
class PureFinancialFactorCalculator(FactorBase, DataProcessingMixin, ColumnMapperMixin):
    ...
    def _register_all_factors(self) -> Dict[str, callable]:
        ...
        methods.update({
            ...
            'ProfitCost_ttm': self.calculate_ProfitCost_ttm,
        })
    ...
    def calculate_ProfitCost_ttm(self, financial_data: pd.DataFrame, **kwargs) -> pd.Series:
        """
        ...
        -----------
        financial_data : pd.DataFrame
            财务数据，包含DEDUCTEDPROFIT、FIN_EXP_IS、TAX等字段
        **kwargs : dict
            可选参数，包括release_dates, trading_dates等
