"""
通过个股PE反推计算行业板块估值指标
Sector Valuation Calculator Using Individual Stock PE Ratios

功能说明：
1. 使用已计算好的个股PE_ttm和PB指标
2. 通过市值/PE反推个股净利润
3. 汇总计算行业板块的整体PE、PB等估值指标

计算公式：
- 个股净利润_TTM = 个股市值 / 个股PE_TTM
- 板块PE_TTM = Σ(个股市值) / Σ(个股净利润_TTM)
- 板块PB = Σ(个股市值) / Σ(个股净资产)
"""
...
# 注意：不从factors模块导入任何内容，避免循环引用
# 本处理器完全独立，只依赖已计算好的个股PE数据

class SectorValuationFromStockPE:
    """
    通过个股PE反推计算行业板块估值
    避免重复计算TTM和日频扩展，提高效率
    """
    ...
    def load_data(self) -> Dict:
        ...
        elif ep_ttm_path.exists():
            self.logger.info("未找到PE_ttm，从EP_ttm转换")
            ep_ttm = pd.read_pickle(ep_ttm_path)
            # EP转PE：PE = 1/EP，注意EP是earnings yield（盈利收益率）
            data['pe_ttm'] = 1 / ep_ttm
            # 处理无穷大和负值
            data['pe_ttm'] = data['pe_ttm'].replace([np.inf, -np.inf], np.nan)
            # 剔除不合理的PE值（负值和过大值）
            data['pe_ttm'][data['pe_ttm'] < 0] = np.nan
            data['pe_ttm'][data['pe_ttm'] > 1000] = np.nan
            self.logger.info(f"从EP_ttm转换得到PE_ttm，形状: {data['pe_ttm'].shape}")
        else:
            self.logger.warning("未找到PE_ttm或EP_ttm数据")
        ...
        if pb_path.exists():
            ...
            self.logger.info("未找到PB，从BP转换")
            bp = pd.read_pickle(bp_path)
            # BP转PB：PB = 1/BP，注意BP是book-to-price ratio
            data['pb'] = 1 / bp
            # 处理无穷大和负值
            data['pb'] = data['pb'].replace([np.inf, -np.inf], np.nan)
            # 剔除不合理的PB值
            data['pb'][data['pb'] < 0] = np.nan
            data['pb'][data['pb'] > 100] = np.nan
            self.logger.info(f"从BP转换得到PB，形状: {data['pb'].shape}")
        else:
            self.logger.warning("未找到PB或BP数据")
    ...
    def process(self, date_range: int = 252) -> pd.DataFrame:
        ...
        self.logger.info("=" * 60)
        self.logger.info("通过个股PE反推计算板块估值")
        self.logger.info("=" * 60)
        ...
        if not sector_valuation.empty:
            # 保存pkl格式
            output_path = self.sector_data_path / "sector_valuation_from_stock_pe.pkl"
            sector_valuation.to_pickle(output_path)
            self.logger.info(f"板块估值数据已保存至: {output_path}")

            # 保存CSV格式
            csv_path = self.sector_data_path / "sector_valuation_from_stock_pe.csv"
    ...
    def generate_summary_report(self, sector_valuation: pd.DataFrame):
        ...
        # 保存汇总报告
        import json
        summary_path = self.sector_data_path / "sector_valuation_summary.json"
